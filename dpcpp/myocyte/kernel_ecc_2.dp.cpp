...
//	MAIN FUNCTION
//=====================================================================
SYCL_EXTERNAL void kernel_ecc_2(fp timeinst, fp *d_initvalu, fp *d_finavalu,
	...
	// I_nak: Na/K Pump Current
        ...
        I_nak_junc = Fjunc * IbarNaK * fnak * Ko /
                     (1 + sycl::pown((KmNaip / d_initvalu_32), 4)) /
                     (Ko + KmKo);
	...
	// I_ks: Slowly Activating K Current
        ...
        eks = (1 / FoRT) * sycl::log((Ko + pNaK * Nao) /
        ...
        I_ks_junc = Fjunc * gks_junc * d_initvalu_12 * d_initvalu_12 *
                    (d_initvalu_39 - eks);
	...
	//
        ...
        ibark = pK * (d_initvalu_39 * Frdy * FoRT) *
                (0.75 * d_initvalu_35 * sycl::exp(d_initvalu_39 * FoRT) -
                 0.75 * Ko) /
                (sycl::exp(d_initvalu_39 * FoRT) - 1);
        ibarna_j = pNa * (d_initvalu_39 * Frdy * FoRT) *
                   (0.75 * d_initvalu_32 * sycl::exp(d_initvalu_39 * FoRT) -
                    0.75 * Nao) /
                   (sycl::exp(d_initvalu_39 * FoRT) - 1);
        ibarna_sl = pNa * (d_initvalu_39 * Frdy * FoRT) *
                    (0.75 * d_initvalu_33 * sycl::exp(d_initvalu_39 * FoRT) -
                     0.75 * Nao) /
                    (sycl::exp(d_initvalu_39 * FoRT) - 1);
	...
	d_finavalu[offset_43]=-I_Ca*Cmem/(Vmyo*2*Frdy)*1e3;
        I_CaK =
            (ibark * d_initvalu_4 * d_initvalu_5 *
	...
	// I_ncx: Na/Ca Exchanger flux
        ...
        s1_junc = sycl::exp(nu * d_initvalu_39 * FoRT) *
                  sycl::pown(d_initvalu_32, 3) * Cao;
        s1_sl = sycl::exp(nu * d_initvalu_39 * FoRT) *
                sycl::pown(d_initvalu_33, 3) * Cao;
        s2_junc = sycl::exp((nu - 1) * d_initvalu_39 * FoRT) *
        ...
        s2_sl = sycl::exp((nu - 1) * d_initvalu_39 * FoRT) *
        ...
        s3_sl = (KmCai * sycl::pown(Nao, 3) *
                 ...
                 sycl::pown(Nao, 3) * d_initvalu_37) *
                (1 + ksat * sycl::exp((nu - 1) * d_initvalu_39 * FoRT));
        I_ncx_junc = Fjunc * IbarNCX * sycl::pow<double>(Q10NCX, Qpow) *
                     Ka_junc * (s1_junc - s2_junc) / s3_junc;
	...
	switch(state){
		...
		case 1:																			// pace w/ current injection at cycleLength 'cycleLength'
                        if (sycl::fmod(timeinst, cycleLength) <= 5) {
                                I_app = 9.5;
			}
			else{
				I_app = 0.0;
