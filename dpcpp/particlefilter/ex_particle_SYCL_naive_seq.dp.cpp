...
void kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles,
        ...
        int i = item_ct1.get_local_range().get(2) * block_id +
 ...
 sycl::queue &q_ct1 = dev_ct1.default_queue();
	...
	*/
	...
	DPCT1003:2: Migrated API does not return error code. (*, 0) is
         * inserted. You may need to rewrite this code.
	*/
        check_error((arrayY_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
        /*
	...
	*/
	...
	for(k = 1; k < Nfr; k++){
		...
		//CUDA memory copying from CPU memory to GPU memory
                q_ct1.memcpy(arrayX_GPU, arrayX, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(arrayY_GPU, arrayY, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(xj_GPU, xj, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(yj_GPU, yj, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(CDF_GPU, CDF, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(u_GPU, u, sizeof(double) * Nparticles).wait();
                ...
                q_ct1.submit([&](sycl::handler &cgh) {
                        cgh.parallel_for(
                            sycl::nd_range<3>(
                                ...
                                sycl::range<3>(1, 1, threads_per_block)),
                            [=](sycl::nd_item<3> item_ct1) {
                                    kernel(arrayX_GPU, arrayY_GPU, CDF_GPU,
		...
		//CUDA memory copying back from GPU to CPU memory
                q_ct1.memcpy(yj, yj_GPU, sizeof(double) * Nparticles).wait();
                q_ct1.memcpy(xj, xj_GPU, sizeof(double) * Nparticles).wait();
