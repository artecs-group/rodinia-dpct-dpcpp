...
*/
void static put_bits_atomic2(unsigned int* out, unsigned int kc,
	...
	cw32 = cw32<<restbits;
        /*
	DPCT1039:10: The generated code assumes that "&out[kc]" points
         * to the global memory address space. If it points to a local memory
         * address space, replace "dpct::atomic_fetch_or" with
         * "dpct::atomic_fetch_or<unsigned int,
         * sycl::access::address_space::local_space>".
	*/
        sycl::atomic<unsigned int>(sycl::global_ptr<unsigned int>(&out[kc]))
            .fetch_or(cw32);
}
...
*/
void static put_bits_atomic2a(unsigned int* out, unsigned int kc,
								unsigned int startbit, unsigned int numbits,
								unsigned int codeword) {
	unsigned int cw32 = codeword;
	unsigned int restbits = 32-startbit-numbits;
	...
	if (startbit == 0 && restbits == 0) {
		...
		DPCT1039:11: The generated code assumes that
                 * "&out[kc]" points to the global memory address space. If it
                 * points to a local memory address space, replace
                 * "dpct::atomic_fetch_or" with "dpct::atomic_fetch_or<unsigned
                 * int, sycl::access::address_space::local_space>".

                 */
                sycl::atomic<unsigned int>(
                    sycl::global_ptr<unsigned int>(&out[kc]))
                    .fetch_or(cw32);
        }
}
