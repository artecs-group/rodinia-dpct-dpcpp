...
#if REORDER_TREE
dpct::image_wrapper<sycl::uint4, 2> nodetex;
dpct::image_wrapper<sycl::uint4, 2> childrentex;
#else
...
#else
/*
DPCT1059:6: SYCL only supports 4-channel image format. Adjust the code.
*/
dpct::image_wrapper<cl::sycl::char4, 1> reftex;
#endif

/*
DPCT1059:7: SYCL only supports 4-channel image format. Adjust the code.
*/
dpct::image_wrapper<cl::sycl::char4, 1> qrytex;

 struct __dpct_align__(8) _MatchCoord
 {
 	union
	{
                sycl::int2 data{};
...
void arrayToAddress(sycl::uchar3 arr, unsigned int &addr)
{
#if REORDER_TREE
...
#if CHILDTEX
#if REORDER_TREE
  return childrentex.read(cur & 0x0000FFFF, (cur & 0xFFFF0000) >> 16);
#else
                       ...
                       )
{
  if (qry_match_len > min_match_len)
  {
    edge_match_length |= rc;
        result->data = sycl::int2(cur, edge_match_length);
...
{
    int qryid = sycl::mul24((unsigned int)item_ct1.get_group(2),
                            (unsigned int)item_ct1.get_local_range(2)) +
        ...
        while ((c != '\0')) 
        {
            ...
            switch (c) 
            {
                ...
                case 'T': next = children.t; break;
                                default: next = sycl::uchar3(0, 0, 0); break;
            };
            ...
            XPRINTF(" Edge coordinates: %d - %d\n", node_start, node_end);
            {
                ...
                if (mustmatch) {
                    refpos = sycl::min(edge_matchlen, past_node_end);
                    qry_match_len += sycl::min(edgelen, mustmatch);
                    mustmatch = sycl::max(dist_to_edge_end, 0);
                }
                else {
...
printKernel(MatchInfo * matches,
            ...
            dpct::image_accessor_ext<sycl::uint4, 2> childrentex) 
{
  int matchid = sycl::mul24((unsigned int)item_ct1.get_group(2),
                            (unsigned int)item_ct1.get_local_range(2)) +
                item_ct1.get_local_id(2);
