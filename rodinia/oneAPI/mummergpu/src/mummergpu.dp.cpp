...
#define CUDA_SAFE_CALL(call)                                                   \
    ...
    if (0 != err) {                                                        \
        fprintf(                                                               \
            stderr, "Cuda error in file '%s' in line %i : %d (%s).\n",         \
...
#define BIND_TEX_ARRAY(tex, arr, desc)                                         \
                                       do {                                                                       \
        CUDA_SAFE_CALL((BIND_TEX_ARRAY(nodetex, (cudaArray*)ref->d_node_tex_array,
                                        nodeTextureDesc).attach(BIND_TEX_ARRAY(nodetex, (cudaArray*)ref->d_node_tex_array, 
					nodeTextureDesc), BIND_TEX_ARRAY(nodetex, (cudaArray*)ref->d_node_tex_array, 
					nodeTextureDesc)), 0));
        ++num_bind_tex_calls;													 \
} while(0)
...
#define CUDA_MALLOC(ptr, size)                                                 \
                               do {                                                                       \
        *(&ref->d_ref_array) = (void *)queries->d_tex_array =                  \
            (char *)queries->d_addrs_tex_array = queries->d_lengths_array =    \
                ctx->results.d_match_coords = d_matches =                      \
                    (MatchInfo *)d_alignments =                                \
                        (struct Alignment *)sycl::malloc_device<MatchCoord>(   \
                            ptr, size, q_ct1, q_ct1, q_ct1, q_ct1, q_ct1,      \
                            q_ct1, q_ct1);                                     \
        ++num_bind_tex_calls;                                                  \
} while (0)
...
#define CUDA_MALLOC_ARRAY(ptr, desc, pitch, rows)                              \
                                                  do {                                                                       \
        *(&ref->d_node_tex_array) = (void *)*(&ref->d_children_tex_array) =    \
            (void *)new dpct::image_matrix(ptr, desc, pitch, rows);            \
        ++num_bind_tex_calls;                                                  \
} while (0)
...
void loadReferenceTexture(MatchContext *ctx) try {
    ...
    dpct::image_channel refTextureDesc =
        /*
        DPCT1059:10: SYCL only supports 4-channel image format. Adjust the code.
        */
        dpct::image_channel(8, 0, 0, 0,
                            dpct::image_channel_data_type::signed_int);
    ...
    if (!ctx->on_cpu) {
        ...
        ctx->ref->bytes_on_board += numrows * ref->pitch;
                ...
                */
                CUDA_SAFE_CALL((
                    q_ct1.memcpy((void *)(ref->d_ref_array), ref->str, ref->len)
                        .wait(),
                    0));
...
void unloadReferenceString(Reference *ref) try {
#if REFTEX
   ...
   CUDA_SAFE_CALL(cudaFreeArray((cudaArray*)(ref->d_ref_array)));
        ...
        */
        CUDA_SAFE_CALL(
            (sycl::free((ref->d_ref_array), dpct::get_default_queue()), 0));
#endif
...
void unloadReferenceTree(MatchContext *ctx) try {
   Reference* ref = ctx->ref;

#if REORDER_TREE
   // Unload nodetex
	#if NODETEX
           CUDA_SAFE_CALL((nodetex.detach(), 0));
           /*
           DPCT1003:16: Migrated API does not return error code. (*, 0) is
           inserted. You may need to rewrite this code.
           */
           CUDA_SAFE_CALL((delete (dpct::image_matrix *)(ref->d_node_tex_array), 0));
#else
    ...
    {
	#if CHILDTEX
           CUDA_SAFE_CALL((childrentex.detach(), 0));
           /*
           DPCT1003:17: Migrated API does not return error code. (*, 0) is
           inserted. You may need to rewrite this code.
           */
           CUDA_SAFE_CALL(
               (delete (dpct::image_matrix *)(ref->d_children_tex_array), 0));
#else
...
void loadReference(MatchContext *ctx) try {
    ...
    if (!ctx->on_cpu) {
...
#if NODETEX
        dpct::image_channel nodeTextureDesc = dpct::image_channel(
            32, 32, 32, 32, dpct::image_channel_data_type::unsigned_int);

        CUDA_MALLOC_ARRAY(nodeTextureDesc,
                          sycl::range<2>(ref->tex_width, ref->tex_node_height));
        ...
        */
        CUDA_SAFE_CALL((
            dpct::dpct_memcpy(
                (dpct::image_matrix *)(ref->d_node_tex_array)
                    ->to_pitched_data(),
                sycl::id<3>(0, 0, 0),
                dpct::pitched_data(
                    ...
                    1),
                sycl::id<3>(0, 0, 0),
                sycl::range<3>(ref->tex_width * ref->tex_node_height *
                                   sizeof(PixelOfNode),
                               1, 1)),
            0));
        ...
        nodetex.set(
            ...
            sycl::coordinate_normalization_mode::
                unnormalized); // access with normalized texture coordinates

        BIND_TEX_ARRAY(nodetex, (dpct::image_matrix *)ref->d_node_tex_array,
                       nodeTextureDesc);
#else
		size_t nodepitch;
        ...
        {
#if CHILDTEX
                dpct::image_channel childrenTextureDesc = dpct::image_channel(
                    32, 32, 32, 32,
                    dpct::image_channel_data_type::unsigned_int);
        CUDA_MALLOC_ARRAY(
            childrenTextureDesc,
            sycl::range<2>(ref->tex_width, ref->tex_children_height));
        ...
        */
        CUDA_SAFE_CALL(
            (dpct::dpct_memcpy(
                 (dpct::image_matrix *)(ref->d_children_tex_array)
                     ->to_pitched_data(),
                 sycl::id<3>(0, 0, 0),
                 dpct::pitched_data(ref->h_children_tex_array,
                                    ...
                                    1),
                 sycl::id<3>(0, 0, 0),
                 sycl::range<3>(ref->tex_width * ref->tex_children_height *
                                    sizeof(PixelOfChildren),
                                1, 1)),
             0));
        ...
        /*
        DPCT1064:20: Migrated cudaBindTextureToArray call is used in a macro
...
void loadQueries(MatchContext *ctx) try {
    dpct::device_ext &dev_ct1 = dpct::get_current_device();
    sycl::queue &q_ct1 = dev_ct1.default_queue();
    ...
    if (!ctx->on_cpu) {
        ...
        */
        CUDA_SAFE_CALL(
            (q_ct1
                 .memcpy((void *)queries->d_tex_array,
        ...
        */
        CUDA_SAFE_CALL(
            (q_ct1
                 .memcpy((void *)queries->d_addrs_tex_array,
                         queries->h_addrs_tex_array, numQueries * sizeof(int))
                 .wait(),
             0));
        ...
        */
        CUDA_SAFE_CALL(
            (q_ct1
                 .memcpy((void *)queries->d_lengths_array,
                         queries->h_lengths_array, numQueries * sizeof(int))
                 .wait(),
             0));
            stopTimer(toboardtimer);
...
void unloadQueries(MatchContext *ctx) try {
    dpct::device_ext &dev_ct1 = dpct::get_current_device();
    sycl::queue &q_ct1 = dev_ct1.default_queue();
   QuerySet* queries = ctx->queries;
   ...
   may need to rewrite this code.
   */
   CUDA_SAFE_CALL((sycl::free(queries->d_tex_array, q_ct1), 0));
   queries->d_tex_array = NULL;
   ...
   may need to rewrite this code.
   */
   CUDA_SAFE_CALL((sycl::free(queries->d_addrs_tex_array, q_ct1), 0));
   queries->d_addrs_tex_array = NULL;
   ...
   may need to rewrite this code.
   */
   CUDA_SAFE_CALL((sycl::free(queries->d_lengths_array, q_ct1), 0));
   queries->d_lengths_array = NULL;
...
void loadResultBuffer(MatchContext *ctx) try {
    dpct::device_ext &dev_ct1 = dpct::get_current_device();
    sycl::queue &q_ct1 = dev_ct1.default_queue();
    ...
    if (!ctx->on_cpu) {
        ...
        */
        CUDA_SAFE_CALL((q_ct1
                            .memset((void *)ctx->results.d_match_coords, 0,
...
void unloadResultBuffer(MatchContext *ctx) try {
    /*
    DPCT1003:28: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((
        sycl::free(ctx->results.d_match_coords, dpct::get_default_queue()), 0));
    ctx->results.d_match_coords = NULL;
    ctx->results.bytes_on_board = 0;
...
void transferResultsFromDevice(MatchContext *ctx) try {
   if (!ctx->on_cpu)
   {
          ...
          */
          CUDA_SAFE_CALL(
              (dpct::get_default_queue()
                   .memcpy(ctx->results.h_match_coords,
...
void runPrintKernel(MatchContext *ctx, ReferencePage *page,
    ...
    */
    ...
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((q_ct1.memcpy(d_matches, h_matches, matchesSize).wait(), 0));
    stopTimer(atimer);
    ...
    sycl::range<3> dimBlock(1, 1, blocksize);
    sycl::range<3> dimGrid(1, 1, ceil(numMatches / (float)BLOCKSIZE));
    ...
    */
    q_ct1.submit([&](sycl::handler &cgh) {
        auto nodetex_acc = nodetex.get_access(cgh);
        auto childrentex_acc = childrentex.get_access(cgh);

        auto nodetex_smpl = nodetex.get_sampler();
        auto childrentex_smpl = childrentex.get_sampler();

        auto ctx_queries_d_tex_array_ct3 = ctx->queries->d_tex_array;
        auto ctx_queries_d_addrs_tex_array_ct4 =
            ctx->queries->d_addrs_tex_array;
        auto ctx_queries_d_lengths_array_ct5 = ctx->queries->d_lengths_array;
        auto page_begin_ct6 = page->begin;
        auto page_end_ct7 = page->end;
        auto page_shadow_left_ct8 = page->shadow_left;
        auto page_shadow_right_ct9 = page->shadow_right;
        auto ctx_min_match_length_ct10 = ctx->min_match_length;
        ...
        cgh.parallel_for(
            ...
            [=](sycl::nd_item<3> item_ct1) {
                printKernel(
                    ...
                    ctx_queries_d_tex_array_ct3,
                    ctx_queries_d_addrs_tex_array_ct4,
                    ctx_queries_d_lengths_array_ct5, page_begin_ct6,
                    page_end_ct7, page_shadow_left_ct8, page_shadow_right_ct9,
                    ctx_min_match_length_ct10, item_ct1,
                    dpct::image_accessor_ext<sycl::uint4, 2>(nodetex_smpl,
                                                             nodetex_acc),
                    dpct::image_accessor_ext<sycl::uint4, 2>(childrentex_smpl,
                                                             childrentex_acc));
            });
    });
    ...
    */
    CUDA_SAFE_CALL(
        (q_ct1.memcpy((void *)alignments, (void *)d_alignments, alignmentSize)
             .wait(),
         0));
    dev_ct1.queues_wait_and_throw();
        stopTimer(atimer);
    ...
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((sycl::free(d_alignments, q_ct1), 0));
    /*
    DPCT1003:37: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((sycl::free(d_matches, q_ct1), 0));
}
catch (sycl::exception const &exc) {
  std::cerr << exc.what() << "Exception caught at file:" << __FILE__
            << ", line:" << __LINE__ << std::endl;
...
void matchOnGPU(MatchContext* ctx, bool doRC)
{
	...
	int blocksize = (numQueries > BLOCKSIZE) ? BLOCKSIZE : numQueries;
        ...
        sycl::range<3> dimGrid(1, 1, ceil(numQueries / (float)BLOCKSIZE));
    ...
    if (doRC) {
        ...
        */
        q_ct1.submit([&](sycl::handler &cgh) {
            auto ctx_results_d_match_coords_ct0 = ctx->results.d_match_coords;
            auto ctx_queries_d_tex_array_ct1 = ctx->queries->d_tex_array;
            auto ctx_queries_d_addrs_tex_array_ct2 =
                ctx->queries->d_addrs_tex_array;
            auto ctx_queries_d_lengths_array_ct3 =
                ctx->queries->d_lengths_array;
            auto ctx_min_match_length_ct5 = ctx->min_match_length;

            cgh.parallel_for(sycl::nd_range<3>(dimGrid * dimBlock, dimBlock),
                             [=](sycl::nd_item<3> item_ct1) {
                                 mummergpuRCKernel(
                                     ctx_results_d_match_coords_ct0,
                                     ctx_queries_d_tex_array_ct1,
                                     ctx_queries_d_addrs_tex_array_ct2,
                                     ctx_queries_d_lengths_array_ct3,
                                     numQueries, ctx_min_match_length_ct5);
                             });
        });
    }
    else {
        /*
        DPCT1049:39: The workgroup size passed to the SYCL kernel may exceed the
        limit. To get the device limit, query info::device::max_work_group_size.
        Adjust the workgroup size if needed.
        */
        q_ct1.submit([&](sycl::handler &cgh) {
            ...
            auto ctx_results_d_match_coords_ct0 = ctx->results.d_match_coords;
            auto ctx_queries_d_tex_array_ct1 = ctx->queries->d_tex_array;
            auto ctx_ref_d_ref_array_ct2 = (char *)ctx->ref->d_ref_array;
            auto ctx_queries_d_addrs_tex_array_ct3 =
                ctx->queries->d_addrs_tex_array;
            auto ctx_queries_d_lengths_array_ct4 =
                ctx->queries->d_lengths_array;
            auto ctx_min_match_length_ct6 = ctx->min_match_length;

            cgh.parallel_for(
                sycl::nd_range<3>(dimGrid * dimBlock, dimBlock),
                [=](sycl::nd_item<3> item_ct1) {
                    mummergpuKernel(ctx_results_d_match_coords_ct0,
                                    ctx_queries_d_tex_array_ct1,
                                    ctx_ref_d_ref_array_ct2,
                                    ctx_queries_d_addrs_tex_array_ct3,
                                    ctx_queries_d_lengths_array_ct4, numQueries,
                                    ctx_min_match_length_ct6, item_ct1,
                                    dpct::image_accessor_ext<sycl::uint4, 2>(
                                        nodetex_smpl, nodetex_acc),
                                    dpct::image_accessor_ext<sycl::uint4, 2>(
                                        childrentex_smpl, childrentex_acc));
                });
        });
    }
...
int getFreeDeviceMemory(bool on_cpu) try {
    dpct::device_ext &dev_ct1 = dpct::get_current_device();
    sycl::queue &q_ct1 = dev_ct1.default_queue();
        size_t free_mem = 0;
	size_t total_mem = 0;
	...
	int * p = NULL;
        ...
        inserted. You may need to rewrite this code.
        */
        CUDA_SAFE_CALL((p = sycl::malloc_device<int>(1, q_ct1), 0));
        /*
...
int matchQueries(MatchContext* ctx) {
    assert(sizeof(struct PixelOfNode) == sizeof(sycl::uint4));
    assert(sizeof(struct PixelOfChildren) == sizeof(sycl::uint4));
