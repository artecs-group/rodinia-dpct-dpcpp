...
void cuda_print_double_array(double *array_GPU, size_t size) {
    ...
    dpct::get_default_queue()
        .memcpy(mem, array_GPU, sizeof(double) * size)
        .wait();
...
void find_index_kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, double * weights, int Nparticles,
    ...
    int i =
        item_ct1.get_local_range().get(2) * block_id + item_ct1.get_local_id(2);
...
void normalize_weights_kernel(double * weights, int Nparticles, double* partial_sums, double * CDF, double * u, int * seed,
                              sycl::nd_item<3> item_ct1, double *u1,
                              double *sumWeights) {
    int block_id = item_ct1.get_group(2);
    int i =
        item_ct1.get_local_range().get(2) * block_id + item_ct1.get_local_id(2);
...
void sum_kernel(double* partial_sums, int Nparticles, sycl::nd_item<3> item_ct1) {
    int block_id = item_ct1.get_group(2);
    int i =
        item_ct1.get_local_range().get(2) * block_id + item_ct1.get_local_id(2);
...
void likelihood_kernel(double * arrayX, double * arrayY, double * xj, double * yj, double * CDF, int * ind, int * objxy, double * likelihood, unsigned char * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double* partial_sums,
                       sycl::nd_item<3> item_ct1, double *buffer) {
    int block_id = item_ct1.get_group(2);
    int i =
        item_ct1.get_local_range().get(2) * block_id + item_ct1.get_local_id(2);
    int y;
    ...
    for (unsigned int s = item_ct1.get_local_range().get(2) / 2; s > 0;
         s >>= 1) {
...
void particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
    ...
    */
    check_error(
        (arrayX_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
   ...
   try{
    check_error(
        (arrayY_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
     ...
     * inserted. You may need to rewrite this code.
    */
    check_error((xj_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:4: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((yj_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:5: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((CDF_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:6: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((u_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:7: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error(
        (likelihood_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    ...
    */
    check_error(
        (q_ct1.memset((void *)likelihood_GPU, 0, sizeof(double) * Nparticles)
             .wait(),
         0));
    /*
    ...
    */
    check_error(
        (weights_GPU = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:10: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((I_GPU = (unsigned char *)sycl::malloc_device(
                     sizeof(unsigned char) * IszX * IszY * Nfr, q_ct1),
                 0));
    /*
    DPCT1003:11: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((objxy_GPU = (int *)sycl::malloc_device(
                     sizeof(int) * 2 * countOnes, q_ct1),
                 0));
    /*
    DPCT1003:12: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((ind_GPU = (int *)sycl::malloc_device(
                     sizeof(int) * countOnes * Nparticles, q_ct1),
                 0));
    /*
    DPCT1003:13: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error((seed_GPU = sycl::malloc_device<int>(Nparticles, q_ct1), 0));
    /*
    DPCT1003:14: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error(
        (partial_sums = sycl::malloc_device<double>(Nparticles, q_ct1), 0));
    ...
    */
    check_error(
        (q_ct1.memcpy(I_GPU, I, sizeof(unsigned char) * IszX * IszY * Nfr)
             .wait(),
         0));
    /*
    ...
    */
    check_error((
        q_ct1.memcpy(objxy_GPU, objxy, sizeof(int) * 2 * countOnes).wait(), 0));
    /*
    DPCT1003:17: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error(
        (q_ct1.memcpy(weights_GPU, weights, sizeof(double) * Nparticles).wait(),
         0));
    /*
    ...
    */
    check_error(
        (q_ct1.memcpy(xj_GPU, xj, sizeof(double) * Nparticles).wait(), 0));
    /*
    DPCT1003:19: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error(
        (q_ct1.memcpy(yj_GPU, yj, sizeof(double) * Nparticles).wait(), 0));
    /*
    DPCT1003:20: Migrated API does not return error code. (*, 0) is
     * inserted. You may need to rewrite this code.
    */
    check_error(
        (q_ct1.memcpy(seed_GPU, seed, sizeof(int) * Nparticles).wait(), 0));
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        /*
        DPCT1049:21: The workgroup size passed to the SYCL kernel may
        ...
        */
        q_ct1.submit([&](sycl::handler &cgh) {
            sycl::accessor<double, 1, sycl::access::mode::read_write,
                ...
                buffer_acc_ct1(sycl::range<1>(threads_per_block), cgh);
            ...
            cgh.parallel_for(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                                  ...
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) {
                    likelihood_kernel(arrayX_GPU, arrayY_GPU, xj_GPU, yj_GPU,
        ...
        /*
        DPCT1049:22: The workgroup size passed to the SYCL kernel may
         * exceed the limit. To get the device limit, query
         * info::device::max_work_group_size. Adjust the workgroup size if
         * needed.
        */
        ...
        */
        q_ct1.submit([&](sycl::handler &cgh) {
            sycl::accessor<double, 0, sycl::access::mode::read_write,
                           sycl::access::target::local>
                u1_acc_ct1(cgh);
            sycl::accessor<double, 0, sycl::access::mode::read_write,
                           sycl::access::target::local>
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1) {
                    normalize_weights_kernel(
                        weights_GPU, Nparticles, partial_sums, CDF_GPU, u_GPU,
                        seed_GPU, item_ct1, u1_acc_ct1.get_pointer(),
        ...
        q_ct1.submit([&](sycl::handler &cgh) {
            cgh.parallel_for(
                sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                                  ...
                                  sycl::range<3>(1, 1, threads_per_block)),
                [=](sycl::nd_item<3> item_ct1) {
                    find_index_kernel(arrayX_GPU, arrayY_GPU, CDF_GPU, u_GPU,
    ...
    */
    check_error(
        (q_ct1.memcpy(arrayX, arrayX_GPU, sizeof(double) * Nparticles).wait(),
         0));
    ...
    */
    check_error(
        (q_ct1.memcpy(arrayY, arrayY_GPU, sizeof(double) * Nparticles).wait(),
         0));
    ...
    */
    check_error(
        (q_ct1.memcpy(weights, weights_GPU, sizeof(double) * Nparticles).wait(),
         0));
