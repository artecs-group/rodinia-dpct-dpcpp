...
#include "device.c.dp.cpp" // (in library path specified to compiler)	needed by for device functions
...
int main(int argc, char *argv []){
	...
	mem_size = sizeof(fp) * Ne;																		// get the size of float representation of input IMAGE
        d_I = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //

        // allocate memory for coordinates on DEVICE
        d_iN = (int *)sycl::malloc_device(mem_size_i, dpct::get_default_queue()); //
        dpct::get_default_queue().memcpy(d_iN, iN, mem_size_i).wait();            //
        d_iS = (int *)sycl::malloc_device(mem_size_i, dpct::get_default_queue()); //
        dpct::get_default_queue().memcpy(d_iS, iS, mem_size_i).wait();            //
        d_jE = (int *)sycl::malloc_device(mem_size_j, dpct::get_default_queue()); //
        dpct::get_default_queue().memcpy(d_jE, jE, mem_size_j).wait();            //
        d_jW = (int *)sycl::malloc_device(mem_size_j, dpct::get_default_queue()); //
        dpct::get_default_queue().memcpy(d_jW, jW, mem_size_j).wait();            //

        // allocate memory for partial sums on DEVICE
        d_sums = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue());  //
        d_sums2 = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //

        // allocate memory for derivatives
        d_dN = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //
        d_dS = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //
        d_dW = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //
        d_dE = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //

        // allocate memory for coefficient on DEVICE
        d_c = (float *)sycl::malloc_device(mem_size, dpct::get_default_queue()); //
	...
	//================================================================================80
        ...
        dpct::get_default_queue().memcpy(d_I, image, mem_size).wait();
	...
	// fflush(NULL);
		...
		// performs subsequent reductions of sums
                blocks2[2] = blocks[2]; // original number of blocks
                blocks2[1] = blocks[1];
		...
		mul = 1;														// original multiplier
			...
			// run kernel
                        /*
                        DPCT1049:13: The workgroup size passed to the SYCL
                        ...
                        */
                        dpct::get_default_queue().submit([&](sycl::handler
                                ...
                                cgh.parallel_for(
                                    sycl::nd_range<3>(blocks2 * threads,
                                                      threads),
                                    [=](sycl::nd_item<3> item_ct1) {
                                            reduce(
			...
			// update execution parameters
                        no = blocks2[2]; // get current number of elements
                        if (blocks2[2] == 1) {
                                blocks2[2] = 0;
                        }
			else{
				mul = mul * NUMBER_THREADS;									// update the increment
                                blocks_x = blocks2[2] / threads[2]; // number of blocks
                                if (blocks2[2] % threads[2] !=
                                    0) { // compensate for division remainder
                                         // above by adding one grid
                                        blocks_x = blocks_x + 1;
				}
                                blocks2[2] = blocks_x;
                                blocks2[1] = 1;
		...
		mem_size_single = sizeof(fp) * 1;
                dpct::get_default_queue().memcpy(&total, d_sums, mem_size_single).wait();
                dpct::get_default_queue().memcpy(&total2, d_sums2, mem_size_single).wait();
	...
	//================================================================================80
        ...
        dpct::get_default_queue().memcpy(image, d_I, mem_size).wait();
	...
	free(jE);
        ...
        sycl::free(d_I, dpct::get_default_queue());
        sycl::free(d_c, dpct::get_default_queue());
        sycl::free(d_iN, dpct::get_default_queue());
        sycl::free(d_iS, dpct::get_default_queue());
        sycl::free(d_jE, dpct::get_default_queue());
        sycl::free(d_jW, dpct::get_default_queue());
        sycl::free(d_dN, dpct::get_default_queue());
        sycl::free(d_dS, dpct::get_default_queue());
        sycl::free(d_dE, dpct::get_default_queue());
        sycl::free(d_dW, dpct::get_default_queue());
        sycl::free(d_sums, dpct::get_default_queue());
        sycl::free(d_sums2, dpct::get_default_queue());
