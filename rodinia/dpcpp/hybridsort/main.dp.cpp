			  ...
			  float *resultList, int numElements)
{
	...
	{
                d_input = (float *)sycl::malloc_device(
                    mem_size, dpct::get_default_queue());
                d_output = (float *)sycl::malloc_device(
                    mem_size, dpct::get_default_queue());
                dpct::get_default_queue()
                    .memcpy((void *)d_input, (void *)origList,
...
try{
	...
	sdkStartTimer(&downloadTimer);
                /*
		DPCT1003:24: Migrated API does not return error
                 * code. (*, 0) is inserted. You may need to rewrite this code.

                 */
                checkCudaErrors(
                    (dpct::get_default_queue()
                         .memcpy((void *)resultList, (void *)mergeresult,
                                 numElements * sizeof(float))
                         .wait(),
                     0));
