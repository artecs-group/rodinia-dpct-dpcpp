...
void kernel(sycl::nd_item<3> item_ct1, params_common_change d_common_change,
            params_common d_common, params_unique *d_unique,
            float *in_partial_sum, float *in_sqr_partial_sum,
            float *in_final_sum, float *in_sqr_final_sum, float *denomT,
            float *par_max_val, int *par_max_coo, float *d_in_mod_temp){
	...
	if(d_common_change.frame_no != 0){
		...
		while(ei_new < d_common.in2_sub2_elem){
			...
			if(temp2 < 0){
				temp2 = 0;
                        ...
                        d_unique[bx].d_in2_sqr_sub2[ei_new] = sycl::sqrt(temp2);
		...
		if(tx == 0){
                        ...
                        mean = *in_final_sum /
                               d_common.in_elem; // gets mean (average) value of
                                                 // element in ROI
                        mean_sqr = mean * mean;
                        ...
                        *denomT = sycl::sqrt(float(d_common.in_elem - 1)) * deviation;
                }
		...
		while(ei_new < d_common.mask_conv_rows){
			...
			for(i=0; i<d_common.mask_conv_cols; i++){
				largest_coordinate_current = ei_new*d_common.mask_conv_rows+i;
                                largest_value_current = sycl::fabs(
                                    d_unique[bx].d_mask_conv
